...
write.csv(markers, 
          './results/01-08-00-dge-all-20231102.csv')
...
write.csv(gsva.hallmark, 
          './results/01-08-00-gsva-hallmark.csv')
write.csv(gsva.kegg, 
          './results/01-08-00-gsva-kegg.csv')
write.csv(gsva.gobp, 
          './results/01-08-00-gsva-gobp.csv')
...
## UMAP图
pdf("./results/01-08-02-UMAP.cluster.withlegend.pdf", 
...
pdf("./results/01-08-03-UMAP.ann.withlegend.pdf", 
...
pdf("./results/01-08-03-bar.ann.withlegend.pdf", 
...
pdf("./results/01-08-04-UMAP.virus.withlegend.pdf", 
...
pdf("./results/01-08-04-bar.virus.withlegend.pdf", 
...
## UMAP（分样本）
pdf("./results/01-08-05-UMAP.ann.seperated_by_group.pdf", 
...
DimPlot(combined, group.by = "Ann", split.by = "sample", reduction = "umap", label = FALSE)  + 
    ...
    theme.2
dev.off()
pdf("./results/01-08-05-UMAP.virus.seperated_by_group.pdf", 
    width = 22/2.54, height = 8/2.54)
...
pdf("./results/01-08-06-UMAP.density.seperated_by_group.pdf", 
...
# prop
pdf("./results/01-08-07-flow.prop_by_ann.withlegend.pdf", 
...
pdf("./results/01-08-07-flow.prop_by_virus.withlegend.pdf", 
...
## 气泡图
markers <- read.csv('./results/01-08-00-dge-all-20231102.csv', )
top <- markers %>% 
    group_by(cluster) %>% 
    top_n(2, avg_log2FC)
...
pdf("./results/01-08-08-dot.markers.pdf", 
...
for(i in c(1:length(marker.list))){
    ...
    pdf(paste("./results/01-08-09-volcano.deg-", 
...
pdf("./results/01-08-10-volcano.deg.pdf", 
...
p = pheatmap(gsva.hallmark[rownames(hallmark.class), ], 
             ...
             border=FALSE)
save_pheatmap_pdf(p, "./results/01-08-11-gsva.pdf",
	12, 12)
