...
for(i in c(1:nrow(comparisons))){
	data = read.csv(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-2", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".csv"), row.names = 1)
	...
	saveWorkbook(wb, 
                     file = paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-0", i, 
...
# 4. GO plot
sets = read.table('/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-00-gobp.txt', sep = '\t')
...
for(j in c(2, 3, 4, 7, 10, 11)){
	...
	g1 = ggplot(data, aes(comparison, ID)) +
	     ...
	     theme_cowplot()
	pdf(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-31-gsea-gobp-dotplot-", 
	...
	g2 = ggplot(data, aes(-log10(p.adjust), ID, shape = comparison)) +
	     ...
	     scale_y_discrete(label = substring(levels(data$ID), 0, 60)) +
             scale_color_gradient2(# limits = c(-3.2, 3.2),
                                  ...
                                  high = 'firebrick3') +
	     theme_cowplot()
	pdf(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-32-gsea-gobp-dotplot-", 
	           names(gobp.gsea.dflist)[j], ".pdf"), 
	    width = 12, height = 10)
	print(g2)
	dev.off()
	...
	g3 = ggplot(data, aes(x = ID, y = NES, group = comparison, fill = comparison)) + 
	     ...
	     scale_x_discrete(label = substring(levels(data$ID), 0, 60)) +
             theme_cowplot()
	pdf(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-33-gsea-gobp-barplot-", 
	           names(gobp.gsea.dflist)[j], ".pdf"), 
	    width = 15, height = 10)
	print(g3)
	dev.off()
