...
dim(combined)
comparisons = read.csv('/workdir/wangph/proj/20231102_Jida/SC-3sample/1.9.0.comparison-2.txt', sep = '\t', header = FALSE)
head(comparisons)
...
for(i in c(1:nrow(comparisons))){
	for (cluster in col.pal.ann[, 1]){
		...
		markers = FindMarkers(sub, 
			...
			logfc.threshold = 0)
		write.csv(markers, 
			paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-1", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], '-', cluster, ".csv", sep = ""))
	}
}
...
for(i in c(1:nrow(comparisons))){
	...
	data = c()
	for (cluster in col.pal.ann[, 1]){
		markers = read.csv(paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-1", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], '-', cluster, ".csv", sep = ""),
		...
		pdf(paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-1", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], '-', cluster, ".pdf", sep = ""), 
	...
	p1 = pheatmap(data_wide_log2fc, 
             	...
             	border=FALSE)
	save_pheatmap_pdf(p1, paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-2", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".pos.pdf", sep = ""), 
		20/2.54, 30/2.54)
	...
	p2 = pheatmap(data_wide_log2fc, 
             	...
             	display_numbers = as.matrix(ifelse(data_wide_adjp <= 0.05, "*", "")), 
             	border=FALSE)
	save_pheatmap_pdf(p2, paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-2", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".neg.pdf", sep = ""), 
		20/2.54, 30/2.54)
	...
	p3 = pheatmap(data_wide_log2fc, 
             	...
             	display_numbers = as.matrix(ifelse(data_wide_adjp <= 0.05, "*", "")), 
             	border=FALSE)
	save_pheatmap_pdf(p3, paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-2", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".pdf", sep = ""), 
		20/2.54, 50/2.54)

	data$comparison = paste0(comparisons$V1[i], 'vs', comparisons$V2[i])
	data_all = rbind(data_all, data)
	deg_all = c(deg_all, features)
	write.csv(data, paste("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-2", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".csv", sep = ""))
}

data_all$comparison = paste0(data_all$cluster, ' (', data_all$comparison, ')')
write.csv(data_all, '/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-24-deg-all.csv')
data_all = read.csv('/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-24-deg-all.csv', row.names = 1)
...
p = pheatmap(data_wide_log2fc, 
	...
	border=FALSE,
	gaps_col = c(26, 52))
save_pheatmap_pdf(p, "/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-24-dge-BvsC.pdf", 
	50/2.54, 60/2.54)
...
p = pheatmap(data_wide_log2fc, 
	...
	border=FALSE,
	gaps_col = c(26, 52))
save_pheatmap_pdf(p, "/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-24-dge-all.pdf", 
	50/2.54, 100/2.54)

features =  read.table('/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-10-GeneSelected.txt')
...
p = pheatmap(data_wide_log2fc, 
	...
	border=FALSE,
	gaps_col = c(26, 52))
save_pheatmap_pdf(p, "/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-24-dge-selecetd.pdf", 
	50/2.54, 40/2.54)
