...
for(i in c(1:nrow(comparisons))){
	data = read.csv(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/01-09-0", i, "-dge-", comparisons$V1[i], 'vs', comparisons$V2[i], ".csv"), row.names = 1)
	...
	write.csv(data.frame(hallmark.gsea), 
	          file = paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-4", i, 
...
# 3.Hallmark plot
hallmark.BvsA = read.csv("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-41-gsea-hallmark-BvsA.csv")
hallmark.CvsA = read.csv("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-42-gsea-hallmark-CvsA.csv")
hallmark.BvsC = read.csv("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-43-gsea-hallmark-BvsC.csv")
...
p = pheatmap(hallmark.nes.dflist[rownames(hallmark.class), ], 
             ...
             border=FALSE)
save_pheatmap_pdf(p, 
                  "/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-44-gsea-hallmark.pdf", 
                  8, 12)
...
for(i in c(1:nrow(comparisons))){
	# hallmark
	path <- paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-0", i, 
	...
	save(list = c("hallmark.nes.dflist", "hallmark.adjp.dflist"), 
             file = paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-1", i, 
                       "-gsea-hallmark-", comparisons$V1[i], 'vs', comparisons$V2[i], ".Rdata"))
}

for(i in c(1:nrow(comparisons))){
	# hallmark
	load(paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-1", i, 
	            "-gsea-hallmark-", comparisons$V1[i], 'vs', comparisons$V2[i], ".Rdata"))
	...
	p = pheatmap(hallmark.nes.dflist[rownames(hallmark.class), ], 
             	...
             	border=FALSE)
	save_pheatmap_pdf(p, 
                          paste0("/workdir/wangph/proj/20231102_Jida/SC-3sample/02-10-2", i, 
